...
@section('content')
    ...
    <script>
        let createFrom = $('#create-form').on('submit', function(e){
            e.preventDefault();
            let model = $('#model').val();
            let brand = $('#brand').val();
            ...
            if(model == '' || brand == '' || plateNumber == '' || insuranceDate == '' ){
                ...
                $.ajax({url: "http://127.0.0.1:8000/api/vehicles/create",
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
                        data: JSON.stringify({model, brand, plateNumber, insuranceDate})
                }).done(function(data){
